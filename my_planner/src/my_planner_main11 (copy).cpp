...
void my_planner::PublishAllPtsLocal()
{
	...
	for(int i=0;i<pts_local.size();i++)
	{
		double x = pts_local[i].x;
		double y = pts_local[i].y;
	   ...
	   if (getRobotPose (pose))
	   {
	     ...
	     if(triggered && !pts.empty())
	     {
	    	 ...
	    	 case 1:
	    	 {
	    		...
	    		// if(last_motion_mode==0)
	    			 PublishAllPtsLocal(); //if last mode is

	 /*   		 bool plan_succ_local = Planner_2(c_x, c_y, c_th, pts_local, vx, vy, w, nextWpt_local);
							...
							Waypoints &pts, int nextWpt, double tol_obst_Forw, double tol_obst_lat)
{
	...
	for(it=laser_ps.begin(); it!=laser_ps.end(); it++)
	{
		...
		if(curr2Forw_dist <= tol_obst_Forw)
		{
			...
			while(LCF_diff_angle < -PI) {LCF_diff_angle += 2*PI;}
			if(abs(LCF_diff_angle) < 0.92)  //0.92rad = max(arctan(0.15/0.20), arctan(0.2/0.15))
			{
	...
	if(pts_Obst.size()>0)
	{
		...
		if( pts_Obst_InLat.size() >= 2 )  //2: Default value,  meet obastacles
		{
			...
			ROS_INFO("pts_Obst_Left,right size are:%d, %d",pts_Obst_Left.size(),pts_Obst_Right.size());
			...
			{
				find_points_dis_perpen(A, B, C, pts_Obst_Right[0].x, pts_Obst_Right[0].y, rob_leng/2,
				...
				find_points_dis_parrallel(A, B, C, LeftWpt_track.x, LeftWpt_track.y, rob_leng,
							Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
			...
			{
				find_points_dis_perpen(A, B, C, pts_Obst_Left[0].x, pts_Obst_Left[0].y, rob_leng/2,
											Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
				double theta_c2No1 = atan2(Temp_wpt_1.y - curr_Wp.y, Temp_wpt_1.x - curr_Wp.x);
				double theta_c2No2 = atan2(Temp_wpt_2.y - curr_Wp.y, Temp_wpt_2.x - curr_Wp.x);
				double theta_diff = theta_c2No1 - theta_c2No2;
				...
				find_points_dis_parrallel(A, B, C, RightWpt_track.x, RightWpt_track.y, rob_leng,
							Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
				double dist_c2No1 = sqrt((Temp_wpt_1.x-curr_Wp.x)*(Temp_wpt_1.x-curr_Wp.x)
											+(Temp_wpt_1.y-curr_Wp.y)*(Temp_wpt_1.y-curr_Wp.y));
				double dist_c2No2 = sqrt((Temp_wpt_2.x-curr_Wp.x)*(Temp_wpt_2.x-curr_Wp.x)
...
////////////////////Left
				...
				{
					Way_Left = 1;
					find_points_dis_perpen(A, B, C, pts_Gap_Left[0].x, pts_Gap_Left[0].y, rob_leng/2,
												Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
					double theta_c2No1 = atan2(Temp_wpt_1.y - curr_Wp.y, Temp_wpt_1.x - curr_Wp.x);
					double theta_c2No2 = atan2(Temp_wpt_2.y - curr_Wp.y, Temp_wpt_2.x - curr_Wp.x);
					double theta_diff = theta_c2No1 - theta_c2No2;
...
////////////////////Right
				...
				{
					Way_Right = 1;
					find_points_dis_perpen(A, B, C, pts_Gap_Right[0].x, pts_Gap_Right[0].y, rob_leng/2,
												Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
					double theta_c2No1 = atan2(Temp_wpt_1.y - curr_Wp.y, Temp_wpt_1.x - curr_Wp.x);
					double theta_c2No2 = atan2(Temp_wpt_2.y - curr_Wp.y, Temp_wpt_2.x - curr_Wp.x);
					double theta_diff = theta_c2No1 - theta_c2No2;
				...
				ROS_INFO("Way_Left,right are:%d, %d",Way_Left,Way_Right);
