...
#define NORM_SPEED 0.22   //0.2 m/s
...
private:
    ...
    Obst_Mode_Wpts curr_wpt_mode;  //current wpt mode
    Obst_Mode_Wpts prev_wpt_mode;  //previous wpt mode

    bool my_planner_control;
    bool move_base_control;
    double c_x,c_y,c_th;
...
my_planner::my_planner(tf::TransformListener &tf):tf_(tf),goal_changed(false),get_pose_successful(false),triggered(false)
{
    ...
    client = n_.serviceClient<topic_tools::MuxSelect>("mux_cmd_vel/select");
...
/*    std::vector<int> wp_index =  Floyd(Graph, pts.size(), 5, 1);
    ...
    my_planner_control = true;

    move_base_control = false;
...
void my_planner::PublishAllPtsLocal()
{
	...
	if(curr_wpt_mode.pts_local.size()==0)
		return;
	std::string ns_wps("Local_Wpts");
	for(int i=0;i<curr_wpt_mode.pts_local.size();i++)
	{
		double x = curr_wpt_mode.pts_local[i].x;
		double y = curr_wpt_mode.pts_local[i].y;
	...
	while(ros::ok())
	{
		...
		if(q_size >= 1)
		{
			...
			if(get_pose_successful && triggered)
			{
				...
				printf("wpt_mode.pts_locar are:\n");
				for(int i=0;i<wpt_mode.pts_local.size();i++)
				{
					printf("%dth pt x:%.3f,y:%.3f\n",i,wpt_mode.pts_local[i].x,wpt_mode.pts_local[i].y);
				}
			}
		}
  ...
  while(ros::ok())
  {
      ...
      //ROS_INFO("control loop is running");
	   if (getRobotPose (pose))
	   {
	        ...
	        //PublishMarker( x,  y, "/map", visualization_msgs::Marker::SPHERE, ns_wps,i,true,false,false);
	        PublishMarker(c_x, c_y, "/map", visualization_msgs::Marker::SPHERE, "WptToAvoidObst",0,true,false,false);
	        //ROS_INFO("marker published");
	        curr_wpt_mode = wpt_mode;

	     if(triggered && !pts.empty())
	     {
	    	 if(my_planner_control&&(prev_wpt_mode.motionMode ==0) && (curr_wpt_mode.motionMode==1))
	    	 {
	    	    	srv.request.topic = "base_cmd_vel"; //change to move_base control
	    	    	if (client.call(srv))
	    	    	{
	    	    		ROS_INFO("changed to base_cmd_vel");
	    	    	}
	    	    	else
	    	    	{
	    	    		ROS_ERROR("Failed to call service topic_tools::MuxSelect");
	    	    		//return 1;
	    	    	}

	    	    	goal.target_pose.header.frame_id = "map";
	    	    	goal.target_pose.header.stamp = ros::Time::now();
	    	    	goal.target_pose.pose.position.x = curr_wpt_mode.pts_local[1].x;
	    	    	goal.target_pose.pose.position.y = curr_wpt_mode.pts_local[1].y;
	    	    	tf::Quaternion quaternion;
	    	    	quaternion = tf::createQuaternionFromYaw(c_th);
	    	    	geometry_msgs::Quaternion qMsg;
	    	    	tf::quaternionTFToMsg(quaternion,qMsg);
	    	    	goal.target_pose.pose.orientation = qMsg;
	    	    	move_base_control = true;
	    	    	my_planner_control = false;
	    	    	goal_heading =  c_th;
	    	    	ac->sendGoal(goal);
		      }
	    	 if(move_base_control)
	    	 {
	    		// double diff_th = abs(c_th-goal_heading);
	    		 double goal_x = goal.target_pose.pose.position.x;
	    		 double goal_y = goal.target_pose.pose.position.y;
	    		 double diff_dis = sqrt((c_x - goal_x)*(c_x - goal_x)+(c_y-goal_y)*(c_y-goal_y));
	    		 if( (diff_dis<= DIST_TOLE))
	    		 {
	    		    	srv.request.topic = "my_planner_cmd_vel";  //change to my planner control
	    		    	if (client.call(srv))
	    		    	{
	    		    		ROS_INFO("changed to my_planner_cmd_vel");
	    		    	}
	    		    	else
	    		    	{
	    		    		ROS_ERROR("Failed to call service topic_tools::MuxSelect");
							...
							Waypoints &pts, int nextWpt, double tol_obst_Forw, double tol_obst_lat)
{
	...
	waypoint curr_Wp, prev_Wp, next_Wp;
	curr_Wp.x = c_x;  curr_Wp.y = c_y;  curr_Wp.theta = c_th;
	next_Wp = pts[nextWpt]; prev_Wp = pts[nextWpt-1];
	A = next_Wp.y - curr_Wp.y; B = curr_Wp.x - next_Wp.x; C = next_Wp.x*curr_Wp.y - curr_Wp.x*next_Wp.y;  //X2*Y1 - X1*Y2
	A_p2n = next_Wp.y - prev_Wp.y; B_p2n = prev_Wp.x - next_Wp.x; C_p2n = next_Wp.x*prev_Wp.y - prev_Wp.x*next_Wp.y;
	...
	if(pts_Obst.size()>0)
	{
		...
		if( pts_Obst_InLat.size() >= 2 )  //2: Default value,  meet obastacles
		{
			...
			temp_Point = pts_Obst_InLat[pts_Obst_InLat.size()-1];
			D_p2n = temp_Point.y*A_p2n - temp_Point.x*B_p2n;
			Point_InP2N.y = (A_p2n*D_p2n-B_p2n*C_p2n)/(A_p2n*A_p2n+B_p2n*B_p2n);
			Point_InP2N.x = (-A_p2n*C_p2n-B_p2n*D_p2n)/(A_p2n*A_p2n+B_p2n*B_p2n);

			find_points_dis_parrallel(A_p2n, B_p2n, C_p2n, Point_InP2N.x, Point_InP2N.y, rob_leng+0.2,
			...
			if(abs(nextWpt2LatObst) <= rob_size/2+0.1)  //too close between nextWpt and obstacle
			{
				nextWpt++;
			}
		}
