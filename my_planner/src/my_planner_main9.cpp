...
public:
    ...
    void PublishMarker(double x, double y, std::string frame, uint32_t shape, std::string ns, int32_t id);
...
void my_planner::PublishAllInitWpts()
{
	...
	for(int i=0;i<pts.size();i++)
	{
		double x = pts[i].x;
		double y = pts[i].y;
		PublishMarker( x,  y, "/map", visualization_msgs::Marker::CUBE, ns_wps,i);
	}
}


void my_planner::PublishMarker(double x, double y, std::string frame, uint32_t shape, std::string ns, int32_t id)
{
  ...
  while(ros::ok())
  {
      ...
      //ROS_INFO("control loop is running");
	   if (getRobotPose (pose))
	   {
	        ...
	        PublishMarker(c_x, c_y, "/map", visualization_msgs::Marker::SPHERE, "WptToAvoidObst",0);
