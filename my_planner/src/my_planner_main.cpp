...
public:
    ...
    void control_loop();
    bool Planner(double c_x, double c_y, double c_th, Waypoints pts, double &vx, double &vy, double &w, int &nextWpt);
...
bool my_planner::Planner(double c_x, double c_y, double c_th, Waypoints pts, double &vx, double &vy, double &w, int &nextWpt)
{
	double dir_slope = 0;
	double w_diff = 0;
	double dir_rad = 0;
	...
	if(nextWpt < pts.size())
	{
		...
		if(dist_CurrPt2Waypt > 0.2)			//within 0.2m
		{
			...
			else if( abs(w_diff) < 0.52 && abs(w_diff) > 0.2 )
			{
				if(w_diff > 0)	{w = 0.2;}
				else	{w = -0.2;}
				vx = 0.0;
...
void my_planner::control_loop()
{
  ...
  while(ros::ok())
  {
	   if (getRobotPose (pose))
	   {
	     ...
	     if(Planner(c_x, c_y, c_th, pts, vx, vy, w, nextWpt))
	     {
	    	 cmd.linear.x = vx;
	    	 cmd.linear.y = vy;
	    	 cmd.angular.z = w;
