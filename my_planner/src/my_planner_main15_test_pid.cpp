...
public:
   ...
   void find_points_dis_parrallel(double A1, double B1,double C1,double x0,double y0,double L,
    ...
    double PIDCalcu(PID *p, double Curr_Err, double Deadzone);
...
double my_planner::PIDCalcu(PID *p, double Curr_Err, double Deadzone)
{
	curr_time = ros::Time::now();
	if((curr_time-prev_time).toSec()<=0.05)
	{
		return p->norm_u;
	}
	else {

		if(abs(Curr_Err) < Deadzone) { return 0;}

		double d_Err;

		d_Err = Curr_Err - p->preErr;			//Îó²îÎ¢·Ö

		double control_Y = p->Kp*(Curr_Err - p->preErr) +
							p->Ki*Curr_Err +
							p->Kd*(Curr_Err - 2*p->preErr + p->pre_preErr);
...
double PIDCalcu(double reference, double feedback, double Max_AngSpeed, double UpdateCycle)
{
	double Setpoint_temp = 0.0;
	double StepSize = Max_AngSpeed*UpdateCycle;

	double curr_err = reference - feedback;
	while(curr_err > PI)  {curr_err -= 2*PI;}
	while(curr_err < -PI) {curr_err += 2*PI;}

	if( (unsigned long) (ros::Time::now().toSec()*1000) - pre_ElapTime >= UpdateCycle*1000)
	{
		pre_ElapTime = (unsigned long) (ros::Time::now().toSec()*1000);
		...
		if(dist_CurrPt2Waypt > DIST_TOLE)			//without
		{
			if(abs(dirLOS_diff) <= PI/2 )
			{
				if( abs(dirLOS_diff) >= DIR_TOLE_LOCAL)     //Æ«²îŽóÓÚ
				{
					vx = 0.0;
					vy = 0.0;
					w = PIDCalcu(&PID_Contr_theta, dirLOS_diff,Deadzone);
					//if(dirLOS_diff > 0)	{w = 0.6;}
					//else {w = -0.6;}
				}
				else
			...
			else
			{
				...
				if( abs(diff_Posi180) >= DIR_TOLE_LOCAL)     //Æ«²îŽóÓÚ
				{
					...
					w = PIDCalcu(&PID_Contr_theta, diff_Posi180,Deadzone);
					//if(diff_Posi180 > 0)	{w = 0.6;}
					//else {w = -0.6;}
				}
				else
...
bool my_planner::Planner_Global_PID(double c_x, double c_y, double c_th, Waypoints &pts, double &vx, double &vy, double &w, int &nextWpt)
{
	waypoint  past_Wp;
	waypoint  next_Wp;
	double A, B, C, D;
				...
				else
				{
					if( dirLOS_diff <= 0 )
					{
						...
						if( abs(diff_Posi90) >= DIR_TOLE)     //Æ«²îŽóÓÚ
						{
							...
							w = PIDCalcu(&PID_Contr_theta, diff_Posi90,Deadzone);
							//if(diff_Posi90 > 0)	{w = 0.6;}
							//else {w = -0.6;}
						}
						else
					...
					else
					{
						...
						if( abs(diff_Anti90) >= DIR_TOLE)     //Æ«²îŽóÓÚ
						{
							...
							w = PIDCalcu(&PID_Contr_theta, diff_Anti90,Deadzone);
							//if(diff_Anti90 > 0)	{w = 0.6;}
							//else {w = -0.6;}
						}
						else
	...
	else
	{
		...
		w = PIDCalcu(&PID_Contr_theta, dir_diff_final,Deadzone);
