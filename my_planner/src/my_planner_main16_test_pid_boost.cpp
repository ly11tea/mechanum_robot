...
unsigned long pre_ElapTime;
double Setpoint, pre_Setpoint;
double Input, Output;
PID myPID(&Input, &Output, &Setpoint, (double)0.2, (double)0.05, (double)0.0);


void PIDInit(double curr_th,double SatMAX, double SatMIN)
{
	myPID.SetOutputLimits(SatMIN, SatMAX);
		...
		if(dist_CurrPt2Waypt > DIST_TOLE)			//without
		{
			if(abs(dirLOS_diff) <= PI/2 )
			{
				if( abs(dirLOS_diff) >= DIR_TOLE_LOCAL)     //Æ«²îŽóÓÚ
				{
					...
					w = PIDCalcu(c2n_slope, c_th, MAX_Y, 0.1);
					//if(dirLOS_diff > 0)	{w = 0.6;}
					//else {w = -0.6;}
				}
				else
