...
void my_planner::ObsAvoid_Thread()
{
	...
	while(ros::ok())
	{
		...
		PublishAllInitWpts();//publish all the initial wpts
    	loop_rate.sleep();
...
bool my_planner::Planner_Local(double c_x, double c_y, double c_th, Waypoints &pts, double &vx, double &vy, double &w, int &nextWpt)
{
waypoint  past_Wp;
waypoint  next_Wp;
double A, B, C, D;
...
if(nextWpt < pts.size())
{
	...
	if(dist_CurrPt2Waypt > DIST_TOLE_LOCAL)			//without
	{
		if(dist_CurrPt2Waypt > 1.5)   //>=1.0m
		{
			if( abs(dirLOS_diff) <= PI/2 )
...
void my_planner::control_loop()
{
  ...
  while(ros::ok())
  {
	     ...
	     if(triggered && !pts.empty())
	     {
	    	 switch(wpt_mode.motionMode)
	    	 {
				 case 0:
				 {
					 ...
					 if(nextWpt == Redefine_pts.size()-1)
						 global_redefined = false; //global plan has finished

					// if(nextWpt_local>1 && nextWpt_local<pts_local.size()-1)
					 if(in_obstacle_avoidance==true && nextWpt_local==4)
						 {
						   in_obstacle_avoidance = false;
						   nextWpt_local = 1;
						 }
					 if(in_obstacle_avoidance==false)
					 {
					 ...
					 if(in_obstacle_avoidance==true)
					 {
						  plan_succ_local = Planner_2(c_x, c_y, c_th, pts_local_saved, vx, vy, w, nextWpt_local);
						 if( plan_succ_local )
						 {
							 cmd.linear.x = vx;
							 cmd.linear.y = vy;
	...
	/*				 ros::Time curr_time = ros::Time::now();
					 ros::Time::Duration diff_time = curr_time - time_avoid_obs;
					if(diff_time.toSec()>)*/
					 pts_local = wpt_mode.pts_local;
					 ROS_INFO("pts local size is%d",pts_local.size());

						 PublishAllPtsLocal(); //if last mode is
				     if(last_motion_mode==0 && nextWpt_local==1 && global_redefined)
				     {
				    	 pts_local_saved = pts_local;
		   		...
		   		// bool plan_succ_local = Planner_2(c_x, c_y, c_th, pts_local_saved, vx, vy, w, nextWpt_local);
				     if(in_obstacle_avoidance == true)
				     {
				    	 plan_succ_local = Planner_2(c_x, c_y, c_th, pts_local_saved, vx, vy, w, nextWpt_local);
...
/*					 cmd.linear.x = 0.0;
	    	...
	    	last_motion_mode = wpt_mode.motionMode;
							...
							Waypoints &pts, int &nextWpt, double tol_obst_Forw, double tol_obst_lat)
{
	...
	Obst_Mode_Wpts  mode_Wpts;

	Waypoints pts_AvoObst_Lest;
	Waypoints pts_AvoObst_Right;
	...
	mode_Wpts.pts_local.push_back(curr_Wp);
	mode_Wpts.pts_local.push_back(next_Wp);
	...
	if(pts_Obst.size()>0)
	{
		...
		{
		    mode_Wpts.motionMode = 1;
			sort(pts_Obst_InLat.begin(), pts_Obst_InLat.end(), less_ForwDist);
			if(pts_Obst_InLat[0].c2laser_dist <= rob_size/2+0.02)
			{
				mode_Wpts.motionMode = 2;
				return mode_Wpts;
			}

			double c2n_dist = sqrt((next_Wp.y-curr_Wp.y)*(next_Wp.y-curr_Wp.y) + (next_Wp.x-curr_Wp.x)*(next_Wp.x-curr_Wp.x));
			double nextWpt2LatObst = pts_Obst_InLat[0].forw_dist - c2n_dist;

			if(nextWpt2LatObst > rob_size/2+0.05 && nextWpt == pts.size()-1)
			{
				mode_Wpts.motionMode = 0;
				return mode_Wpts;
			}

			if(abs(nextWpt2LatObst) <= rob_size/2+0.1)  //close between nextWpt and obstacle
			{
				nextWpt++;
			}
			...
			double wptDist_Left = 0;
			double wptDist_Right = 0;
			waypoint LeftWpt_track;
			waypoint RightWpt_track;
			waypoint Temp_wpt_1;
			waypoint Temp_wpt_2;
			...
			else if(pts_Obst_Left.size() <= 2 && pts_Obst_Right.size() >= 3 )
			{
				find_points_dis_perpen(A, B, C, pts_Obst_Right[0].x, pts_Obst_Right[0].y, rob_leng-0.1,
											Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
				double theta_c2No1 = atan2(Temp_wpt_1.y - curr_Wp.y, Temp_wpt_1.x - curr_Wp.x);
				double theta_c2No2 = atan2(Temp_wpt_2.y - curr_Wp.y, Temp_wpt_2.x - curr_Wp.x);
				double theta_diff = theta_c2No1 - theta_c2No2;
				while(theta_diff > PI)  {theta_diff -= 2*PI;}
				while(theta_diff < -PI) {theta_diff += 2*PI;}
				if(theta_diff >= 0)
				{
					LeftWpt_track.x = Temp_wpt_1.x;
					LeftWpt_track.y = Temp_wpt_1.y;
				}
				else
				{
					LeftWpt_track.x = Temp_wpt_2.x;
					LeftWpt_track.y = Temp_wpt_2.y;
				}
				mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+1, LeftWpt_track);

				find_points_dis_parrallel(A, B, C, LeftWpt_track.x, LeftWpt_track.y, tol_obst_Forw_-0.2,
							Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
				double dist_c2No1 = sqrt((Temp_wpt_1.x-curr_Wp.x)*(Temp_wpt_1.x-curr_Wp.x)
											+(Temp_wpt_1.y-curr_Wp.y)*(Temp_wpt_1.y-curr_Wp.y));
				double dist_c2No2 = sqrt((Temp_wpt_2.x-curr_Wp.x)*(Temp_wpt_2.x-curr_Wp.x)
											+(Temp_wpt_2.y-curr_Wp.y)*(Temp_wpt_2.y-curr_Wp.y));
				if(dist_c2No1 <= dist_c2No2)
				{
					mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+1, Temp_wpt_1);
					mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+3, Temp_wpt_2);
				}
				else
				{
					mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+1, Temp_wpt_2);
					mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+3, Temp_wpt_1);
				}

				mode_Wpts.motionMode = 1;
				return mode_Wpts;
			}
			else if(pts_Obst_Left.size() >= 3 && pts_Obst_Right.size() <= 2 )
			{
				find_points_dis_perpen(A, B, C, pts_Obst_Left[0].x, pts_Obst_Left[0].y, rob_leng-0.1,
											Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
				double theta_c2No1 = atan2(Temp_wpt_1.y - curr_Wp.y, Temp_wpt_1.x - curr_Wp.x);
				double theta_c2No2 = atan2(Temp_wpt_2.y - curr_Wp.y, Temp_wpt_2.x - curr_Wp.x);
				double theta_diff = theta_c2No1 - theta_c2No2;
				...
				if(theta_diff > 0)
				{
					RightWpt_track.x = Temp_wpt_2.x;
					RightWpt_track.y = Temp_wpt_2.y;
				}
				else
				{
					RightWpt_track.x = Temp_wpt_1.x;
					RightWpt_track.y = Temp_wpt_1.y;
					
				}
				mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+1, RightWpt_track);

				find_points_dis_parrallel(A, B, C, RightWpt_track.x, RightWpt_track.y, tol_obst_Forw_-0.2,
							Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
				double dist_c2No1 = sqrt((Temp_wpt_1.x-curr_Wp.x)*(Temp_wpt_1.x-curr_Wp.x)
											+(Temp_wpt_1.y-curr_Wp.y)*(Temp_wpt_1.y-curr_Wp.y));
				double dist_c2No2 = sqrt((Temp_wpt_2.x-curr_Wp.x)*(Temp_wpt_2.x-curr_Wp.x)
			...
			{
////////////////////Left
				if(min_LatDist_Left > rob_size && min_LatDist_Right > rob_size)
				{
					mode_Wpts.motionMode = 0;
					return mode_Wpts;
				}
				if(min_LatDist_Left > rob_size)
				{
					find_points_dis_perpen(A, B, C, pts_Obst_Right[0].x, pts_Obst_Right[0].y, rob_leng-0.1,
												Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
					double theta_c2No1 = atan2(Temp_wpt_1.y - curr_Wp.y, Temp_wpt_1.x - curr_Wp.x);
				...
				else if(min_LatDist_Right > rob_size)
				{
					find_points_dis_perpen(A, B, C, pts_Obst_Left[0].x, pts_Obst_Left[0].y, rob_leng-0.1,
												Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
					double theta_c2No1 = atan2(Temp_wpt_1.y - curr_Wp.y, Temp_wpt_1.x - curr_Wp.x);
				...
				for( i=0; i<pts_Obst_Left.size()-1; i++)
				{
					...
					if(abs(neighb_Dist_Left) > rob_size-0.1)
					{
						if(neighb_Dist_Left >=0)
						{
				...
				if(pts_Gap_Left.size() != 0)
				{
					Way_Left = 1;
					find_points_dis_perpen(A, B, C, pts_Gap_Left[0].x, pts_Gap_Left[0].y, rob_leng-0.1,
												Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
					double theta_c2No1 = atan2(Temp_wpt_1.y - curr_Wp.y, Temp_wpt_1.x - curr_Wp.x);
					double theta_c2No2 = atan2(Temp_wpt_2.y - curr_Wp.y, Temp_wpt_2.x - curr_Wp.x);
					double theta_diff = theta_c2No1 - theta_c2No2;
...
////////////////////Right
				for( i=0; i<pts_Obst_Right.size()-1; i++)
				{
					...
					if(abs(neighb_Dist_Right) > rob_size-0.1)
					{
						if(neighb_Dist_Right >=0)
						{
				...
				if(pts_Gap_Right.size() != 0)
				{
					Way_Right = 1;
					find_points_dis_perpen(A, B, C, pts_Gap_Right[0].x, pts_Gap_Right[0].y, rob_leng-0.1,
												Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
					double theta_c2No1 = atan2(Temp_wpt_1.y - curr_Wp.y, Temp_wpt_1.x - curr_Wp.x);
					double theta_c2No2 = atan2(Temp_wpt_2.y - curr_Wp.y, Temp_wpt_2.x - curr_Wp.x);
					double theta_diff = theta_c2No1 - theta_c2No2;
				...
				if( Way_Left == 0  && Way_Right == 0  )
				{
					mode_Wpts.motionMode = 3;
					return mode_Wpts;
				}
				else if(Way_Left == 1  && Way_Right == 0  )
				{
					mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+1, LeftWpt_track);
				...
				else if(Way_Left == 0  && Way_Right == 1  )
				{
					mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+1, RightWpt_track);
					find_points_dis_parrallel(A, B, C, RightWpt_track.x, RightWpt_track.y, tol_obst_Forw_-0.2,
								Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
				...
				{
					wptDist_Left = abs(A*LeftWpt_track.x + B*LeftWpt_track.y + C)/(sqrt(A*A + B*B));
					wptDist_Right = abs(A*RightWpt_track.x + B*RightWpt_track.y + C)/(sqrt(A*A + B*B));
					if(wptDist_Left <= wptDist_Right)
					{
						mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+1, LeftWpt_track);
			...
			if(abs(nextWpt2LatObst) <= rob_size/2)  //close between nextWpt and obstacle
			{

			}
			else if(nextWpt2LatObst < -rob_size/2)
			{
			}
		}
		else
