							...
							Waypoints &pts, int nextWpt, double tol_obst_Forw, double tol_obst_lat)
{
	...
	if(pts_Obst.size()>0)
	{
		...
		if( pts_Obst_InLat.size() >= 3 )  //3: Default value,  meet obastacles
		{
			...
			if(pts_Obst_Left.size() >= 2)
			{
				for(int i=0; i<pts_Obst_Left.size()-1; i++)
				{
					double neighb_Dist_Left = abs(pts_Obst_Left[i+1].lat_dist - pts_Obst_Left[i].lat_dist);
					if(neighb_Dist_Left > rob_size)
					{
				...
				if(pts_Gap_Left.size() >= 2)
				{
					LeftWpt_track.x = (pts_Gap_Left[0].x + pts_Gap_Left[1].x)/2 ;
					LeftWpt_track.y = (pts_Gap_Left[0].y + pts_Gap_Left[1].y)/2 ;
			...
			if(pts_Obst_Right.size() >= 2)
			{
				for(int j=pts_Obst_Right.size()-1; j>0; j--)
				{
					double neighb_Dist_Right = abs(pts_Obst_Right[j].lat_dist - pts_Obst_Right[j-1].lat_dist);
					if(neighb_Dist_Right > rob_size)
					{
				...
				if(pts_Gap_Right.size() >= 2)
				{
					RightWpt_track.x = (pts_Gap_Right[0].x + pts_Gap_Right[1].x)/2 ;
					RightWpt_track.y = (pts_Gap_Right[0].y + pts_Gap_Right[1].y)/2 ;
			...
			if(wptDist_Left !=0 && wptDist_Right !=0)
			{

				if(wptDist_Left < wptDist_Right)
				{
					//pts_AvoObst_Lest.push_back(LeftWpt_track);
					mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+1, LeftWpt_track);
					find_points_dis_parrallel(A, B, C, LeftWpt_track.x, LeftWpt_track.y, rob_leng/2,
				...
				{
					//pts_AvoObst_Lest.push_back(RightWpt_track);
					mode_Wpts.pts_local.insert(mode_Wpts.pts_local.begin()+1, RightWpt_track);

					find_points_dis_parrallel(A, B, C, RightWpt_track.x, RightWpt_track.y, rob_leng/2,
							Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
			...
			else if( wptDist_Left ==0  && wptDist_Right !=0  )
			{
				find_points_dis_perpen(A, B, C, pts_Obst[0].x, pts_Obst[0].y, rob_leng/2,
											Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
				double dist_c2No1 = sqrt((Temp_wpt_1.x-curr_Wp.x)*(Temp_wpt_1.x-curr_Wp.x)
											+(Temp_wpt_1.y-curr_Wp.y)*(Temp_wpt_1.y-curr_Wp.y));
				double dist_c2No2 = sqrt((Temp_wpt_2.x-curr_Wp.x)*(Temp_wpt_2.x-curr_Wp.x)
...
{// find the two points whose distance to point(x0,y0) are L and who are on the line that is penpendicular to the line represented by (A1*x + B1*y+C1=0)
	...
	double	x0_x0 = x0*x0;
	double	x0_y0 = x0*y0;
	x1 = -A*(C + (B*(A_A*y0 - B_C + A*sqrt((A_A*L_L - A_A*x0_x0 - 2*A_B*x0_y0 - 2*A_C*x0
		+ B_B*L_L - B_B*y0*y0 - 2*B_C*y0 - C_C)) - A_B*x0))/(square_A_B));
	x2 = -A*(C - (B*(B_C - A_A*y0 + A*sqrt((A_A*L_L - A_A*x0_x0 - 2*A_B*x0_y0 - 2*A_C*x0
		+ B_B*L_L - B_B*y0*y0 - 2*B_C*y0 - C_C)) + A_B*x0))/(square_A_B));
	y1 =  (A_A*y0 - B_C + A*sqrt((A_A*L_L - A_A*x0_x0 - 2*A_B*x0_y0 - 2*A_C*x0 + B_B*L_L
		- B_B*y0*y0 - 2*B_C*y0 - C_C)) - A_B*x0)/(square_A_B);
	y2 = -(B_C - A_A*y0 + A*sqrt((A_A*L_L - A_A*x0_x0 - 2*A_B*x0_y0 - 2*A_C*x0 + B_B*L_L
		- B_B*y0*y0 - 2*B_C*y0 - C_C)) + A_B*x0)/(square_A_B);
}
