...
typedef struct PID{
	 ...
	 double pre_preErr;
	 double preErr;			//Ç°Ò»ÅÄÎó²î
	 double sumErr;			//Îó²îÀÛ»ý

	 double norm_u;

	 double MIN_Y;		//×îÐ¡ÏÞ·ù
	 double MAX_Y;		//×îŽóÏÞ·ù
...
private:
    ...
    double Setpoint, pre_Setpoint;//pay attention to their initialization
    double update_cycle;
...
public:
   ...
   void find_points_dis_parrallel(double A1, double B1,double C1,double x0,double y0,double L,
    ...
    double PIDCalcu(PID *p, double reference, double feedback, double Max_AngSpeed, double UpdateCycle, double Deadzone);


    void PIDInit(PID *p, double Kp, double Ki, double Kd, double min_Y, double max_Y);

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    bool Planner_Local_PID(double c_x, double c_y, double c_th, Waypoints &pts_Local,
    		int &nextWpt_Local, double &vx, double &vy, double &w);

    bool Planner_Local_PID_1(double c_x, double c_y, double c_th,
    		Waypoints &pts_Local, int &nextWpt_Local, double &vx, double &vy, double &w);

    bool Planner_Global_PID(double c_x, double c_y, double c_th, Waypoints &pts,
    		double &vx, double &vy, double &w, int &nextWpt);
...
my_planner::my_planner(tf::TransformListener &tf):tf_(tf),goal_changed(false),get_pose_successful(false),triggered(false)
{
    ...
    n_local.param("KP",KP, 0.25);
    n_local.param("KI",KI, 0.05);
    n_local.param("KD",KD, 0.00);
    n_local.param("MIN_Y",MIN_Y, -0.8);
    n_local.param("MAX_Y",MAX_Y, 0.8);
    n_local.param("Deadzone",Deadzone, 0.05);
    n_local.param("update_cycle",update_cycle, 0.05);
...
void my_planner::control_loop()
{
  ...
  PIDInit(&PID_Contr_theta, KP, KI, KD, MIN_Y, MAX_Y);
  while(ros::ok())
  {
		  ...
		  if(triggered && !pts.empty())
		  {
			  ...
			  if(task_finished==false)
			  {
				  ...
				  if(Curr_state.obst_exist_C2N == false  && Global_Planner == 1)
				  {
					  ...
					  //Planner_Global_PID(c_x, c_y, c_th, Redefine_pts, vx, vy, w, nextWpt);
				  ...
				  else if(Curr_state.obst_exist_C2N == true && Global_Planner == 1  )
				  {
					  ...
					  else
					  {
						  ...
						  pre_Setpoint = c_th;
						  //Planner_Local_PID(c_x, c_y, c_th, wpts_Local.pts_local, nextWpt_Local, vx, vy, w);
						  Local_Planner = 1;
						  Global_Planner = 0;
						  next_point = wpts_Local.pts_local[nextWpt_Local];
					  }
	...
	if(pts_Obst.size()>0)
	{
		...
		if( pts_Obst_InLat.size() >= 2 )  //2: Default value,  meet obastacles
		{
			...
			else
			{
				...
				{
					find_points_dis_perpen(A, B, C, pts_Obst_Left[0].x, pts_Obst_Left[0].y, LAT_DIST_OBS,
				...
				{
					find_points_dis_perpen(A, B, C, pts_Obst_Right[0].x, pts_Obst_Right[0].y, LAT_DIST_OBS,
											Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
					TempDist_1 = getDist_P2L(Temp_wpt_1.x, Temp_wpt_1.y, A, B, C);
					TempDist_2 = getDist_P2L(Temp_wpt_2.x, Temp_wpt_2.y, A, B, C);
					if(TempDist_1 <= TempDist_2)
				...
				else
				{
					...
					{
						if(min_LatDist_Left <= min_LatDist_Right )
						{
							find_points_dis_perpen(A, B, C, pts_Obst_Left[0].x, pts_Obst_Left[0].y, LAT_DIST_OBS,
												Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
						}
					...
					else
					{
						...
						if(pts_Gap_Left.size() != 0 && pts_Gap_Right.size() != 0)
						{
							...
							{
								find_points_dis_perpen(A, B, C, pts_Gap_Left[0].x, pts_Gap_Left[0].y, LAT_DIST_OBS,
												Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
							}
							else
							{
								find_points_dis_perpen(A, B, C, pts_Gap_Right[0].x, pts_Gap_Right[0].y, LAT_DIST_OBS,
												Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
							}
						}
						else if(pts_Gap_Left.size() == 0 && pts_Gap_Right.size() == 0)
						{
							...
							{
								find_points_dis_perpen(A, B, C, pts_Obst_Left[pts_Obst_Left.size()-1].x, pts_Obst_Left[pts_Obst_Left.size()-1].y,
									LAT_DIST_OBS, Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
							}
							else
							{
								find_points_dis_perpen(A, B, C, pts_Obst_Right[pts_Obst_Right.size()-1].x, pts_Obst_Right[pts_Obst_Right.size()-1].y,
									LAT_DIST_OBS, Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
							}
						}
						else if(pts_Gap_Left.size() != 0 && pts_Gap_Right.size() == 0)
...
Avoid_Obst_Wpts my_planner::FindWptToAvoidObst( obstacleState Obst_State, double c_x, double c_y, double c_th, waypoint prev_Wpt, waypoint next_Wpt)
{
	...
	find_points_dis_parrallel(A_p2n, B_p2n, C_p2n, Obst_State.feasible_Wpt.x, Obst_State.feasible_Wpt.y, rob_size+0.2,
								Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
	...
	valid_Wpts.pts_local.insert(valid_Wpts.pts_local.end()-1,  Obst_State.feasible_Wpt);


	/*valid_Wpts.pts_local.push_back(Obst_State.feasible_Wpt);

	find_points_dis_parrallel(A_p2n, B_p2n, C_p2n, Obst_State.feasible_Wpt.x, Obst_State.feasible_Wpt.y, rob_size,
								Temp_wpt_1.x, Temp_wpt_2.x, Temp_wpt_1.y, Temp_wpt_2.y);
	double dist_c2No1 = sqrt((Temp_wpt_1.x-curr_Wpt.x)*(Temp_wpt_1.x-curr_Wpt.x)
											+(Temp_wpt_1.y-curr_Wpt.y)*(Temp_wpt_1.y-curr_Wpt.y));
	double dist_c2No2 = sqrt((Temp_wpt_2.x-curr_Wpt.x)*(Temp_wpt_2.x-curr_Wpt.x)
...
bool my_planner::Planner_Local(double c_x, double c_y, double c_th, Waypoints &pts_Local, int &nextWpt_Local, double &vx, double &vy, double &w)
{
	...
	if(nextWpt_Local < pts_Local.size()-1)
	{
		...
		if(dist_CurrPt2Waypt > DIST_TOLE)			//without
		{
			if(abs(dirLOS_diff) <= PI/2 )
			{
				if( abs(dirLOS_diff) >= DIR_TOLE_LOCAL)     //ƫ\B2\EE\B4\F3\D3\DA
				{
					vx = 0.0;
					vy = 0.0;
					if(dirLOS_diff > 0)	{w = BIG_REGU;}
					else {w = -BIG_REGU;}
				}
				else
				{
					...
					if(abs(dirLOS_diff) > DIR_TOLE_LOCAL/3)			// DIR_TOLE/3
					{
			...
			else
			{
				...
				if( abs(diff_Posi180) >= DIR_TOLE_LOCAL)     //ƫ\B2\EE\B4\F3\D3\DA
				{
					vx = 0.0;
					vy = 0.0;
				...
				else
				{
					...
					if(abs(diff_Posi180) > DIR_TOLE_LOCAL/3)			//
					{
...
double my_planner::PIDCalcu(PID *p, double reference, double feedback, double Max_AngSpeed, double UpdateCycle, double Deadzone)
{
	double Setpoint_temp = 0.0;
	double StepSize = PI/3;

	double curr_err = reference - feedback;
	while(curr_err > PI)  {curr_err -= 2*PI;}
	while(curr_err < -PI) {curr_err += 2*PI;}
	curr_time = ros::Time::now();

	if( (curr_time-prev_time).toSec() >= UpdateCycle)
		{
		    prev_time = curr_time;

			if( curr_err >= 0 )
			{
				Setpoint_temp = feedback + StepSize;
				while(Setpoint_temp > PI)  {Setpoint_temp -= 2*PI;}
				while(Setpoint_temp < -PI) {Setpoint_temp += 2*PI;}

				if(Setpoint_temp >= reference) {Setpoint = reference;}
				else{ Setpoint = Setpoint_temp; }
			}
			else
			{
				Setpoint_temp = feedback - StepSize;
		...
		{
			Setpoint = pre_Setpoint;
		}


	   double M_Curr_Err = Setpoint - feedback;

		if(abs(M_Curr_Err) < Deadzone) { return 0;}

		double d_Err;

		d_Err = M_Curr_Err - p->preErr;			//Îó²îÎ¢·Ö

		double control_Y = p->Kp*(M_Curr_Err - p->preErr) +
							p->Ki*M_Curr_Err +
							p->Kd*(M_Curr_Err - 2*p->preErr + p->pre_preErr);

		p->norm_u += control_Y;

		if(p->norm_u < p->MIN_Y) { p->norm_u = p->MIN_Y; }
		if(p->norm_u > p->MAX_Y) { p->norm_u = p->MAX_Y; }

		p->pre_preErr = p->preErr ;
		p->preErr = M_Curr_Err;
     //   prev_time = curr_time;
...
void my_planner::PIDInit(PID *p, double Kp, double Ki, double Kd, double min_Y, double max_Y)
{
	p->reference = 0;
	p->Kp = Kp;  p->Ki = Ki;  p->Kd = Kd;
	p->pre_preErr = 0;
	p->preErr = 0;    p->sumErr = 0;
	p->norm_u = 0;
	p->MIN_Y = min_Y;  p->MAX_Y = max_Y;
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool my_planner::Planner_Local_PID(double c_x, double c_y, double c_th, Waypoints &pts_Local, int &nextWpt_Local, double &vx, double &vy, double &w)
{
	waypoint  past_Wp, next_Wp;
	...
	if(nextWpt_Local < pts_Local.size()-1)
	{
		...
		if(dist_CurrPt2Waypt > DIST_TOLE)			//without
		{
			if( abs(dirLOS_diff) >= DIR_TOLE_LOCAL )
			{
				vx = 0.0;
				vy = 0.0;
				w = PIDCalcu(&PID_Contr_theta, c2n_slope, c_th, MAX_Y, update_cycle, Deadzone);
			}
			else
			{
				vx = NORM_SPEED_LOCAL;
...
bool my_planner::Planner_Local_PID_1(double c_x, double c_y, double c_th, Waypoints &pts_Local, int &nextWpt_Local, double &vx, double &vy, double &w)
{
	waypoint  past_Wp, next_Wp;
