...
bool my_planner::Planner_2(double c_x, double c_y, double c_th, Waypoints pts, double &vx, double &vy, double &w, int &nextWpt)
{
	...
	if(nextWpt < pts.size())
	{
		...
		if(dist_CurrPt2Waypt > DIST_TOLE)			//within 
		{
			...
			p2c_slope = atan2(c_y - past_Wp.y, c_x - past_Wp.x);
			p2n_slope = atan2(next_Wp.y - past_Wp.y, next_Wp.y - past_Wp.x);
			...
			if( abs(abs(dir_diff) - PI) < DIR_TOLE )   //back up
			{
				vx = -NORM_SPEED;
				if(lat_dist > LAT_DIST_TOL)
				{
					if(c2p2n_diff > 0) {vy = -0.05;}
					else {vy = 0.05;}
				}
				else {vy = 0;}				
			...
			else if( abs(abs(dir_diff) - PI/2) < DIR_TOLE && dist_CurrPt2Waypt < 1.5)    //Within PI/2 and within 0.5m
			{
				if(dir_diff > 0)    //Left
				{
					vx = 0;
